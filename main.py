...
class HashedNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc1(x))
        self.fc1.accumulate_metrics(x)  # Accumulate HashedFC1 activations for the weighted average
        x = F.relu(self.fc2(x))
        self.fc2.accumulate_metrics(x)   # Accumulate HashedFC2 activations for the weighted average
        x = F.relu(self.fc3(x))
        self.fc3.accumulate_metrics(x)   # Accumulate HashedFC3 activations for the weighted average
        x = self.fc4(x)
        self.fc4.accumulate_metrics(x)   # Accumulate HashedFC4 activations for the weighted average
        return x
